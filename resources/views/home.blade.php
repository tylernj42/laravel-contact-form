...
<head>
    ...
    <style>
        ...
        form .form-overlay .loading-spinner{
    ...
    <section id="contact" class="container content-section text-center">
        <div class="row">
            <div class="col-lg-8 col-lg-offset-2">
                ...
                <form class="container-fluid text-left" method="POST" action="/contactRequest" @submit.prevent="onSubmit" @keydown="form.errors.clear($event.target.id)">
                    {{ csrf_field() }}
                    <div class="form-group" v-bind:class="errorClass('name')">
                        <label for="name">Full Name</label>
                        <input type="text" class="form-control" id="name" name="name" v-model="form.name">
                        <span class="help-block" v-if="form.errors.has('name')" v-text="form.errors.get('name')"></span>
                    </div>
                    <div class="row">
                        <div class="col-sm-6 form-group" v-bind:class="errorClass('email')">
                    ...
                    <div class="form-group" v-bind:class="errorClass('message')">
                        <label for="message">Message</label>
                        <textarea id="message" name="message" rows="8" class="form-control" v-model="form.message"></textarea>
                        <span class="help-block" v-if="form.errors.has('message')" v-text="form.errors.get('message')"></span>
                    </div>
                    <div class="form-group text-center">
                        <button type="submit" name="submit-contact-request" class="btn btn-default btn-lg" :disabled="form.errors.hasAny()">Submit</button>
                    </div>
                    <transition name="v-fade">
                        <div class="form-overlay" v-if="form.status === 'loading'">
                            <span class="loading-spinner"></span>
                        </div>
                        <div class="form-overlay" v-if="form.status === 'sent'">
                            Your message has been sent. Thank you.
                        </div>
                        <div class="form-overlay" v-if="form.status === 'failed'" v-text="form.errorMessage"></div>
                    </transition>
                </form>
            </div>
        </div>
